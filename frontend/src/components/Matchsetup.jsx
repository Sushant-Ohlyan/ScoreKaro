...
const MatchSetup = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const finalData = {
  overs: Number(oversValue),
  venue: matchDetails.venue,
  ballType: matchDetails.ballType,
  matchType: matchDetails.matchType,
  umpireName1: matchDetails.umpire1,
  umpireName2: matchDetails.umpire2,
  umpireName3: matchDetails.umpire3,
  tossWonBy: matchDetails.tossWinner,
  tossDecision: matchDetails.tossDecision,
};

    try {
      const res = await axios.post("http://localhost:5000/api/match-setup/save", finalData);
      alert(res.data.message);
      console.log("Match Setup Saved:", res.data);
